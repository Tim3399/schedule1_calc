"""Bounded-memory exact search experiment.

Prefix layers merge equivalent ordered effect states. The final one or two layers are
streamed without retaining terminal states, reducing peak memory while preserving exact
winner and tie behavior. Any resource limit raises instead of exposing partial results.
"""
...
class SearchLimitExceeded(RuntimeError):
    """Raised when an exact search cannot finish within its configured limits."""
...
def search(
    ...
    *,
    ...
    expansion_limit: int = 20_000_000,
    frontier_limit: int = 100_000,
    time_limit_seconds: float = 90.0,
) -> tuple[CombinationResult, CombinationResult, dict]:
    """Return exact modifier/profit winners or raise on a configured resource limit."""
    if not isinstance(product_name, str) or not product_name.strip():
        raise ValueError("product_name must be a non-empty string")
    if isinstance(max_size, bool) or not isinstance(max_size, int) or not 1 <= max_size <= 16:
        raise ValueError("max_size must be an integer from 1 through 16")
    ...
    for name, value, minimum in (
        ...
        ("expansion_limit", expansion_limit, 1),
    ...
    def transition(
        ...
        if stats["expanded_transitions"] >= expansion_limit:
            raise SearchLimitExceeded(
                f"Exact search exceeded its {expansion_limit:,} transition limit."
            )
        stats["expanded_transitions"] += 1
        if stats["expanded_transitions"] & 1023 == 0:
            check_time()

        cache_key = (state, ingredient_index)
        if use_cache and cache_key in transition_cache:
    ...
    def evaluate(state: tuple[str, ...], *, use_cache: bool) -> tuple[float, Decimal]:
        ...
        result = modifier, Decimal(base_price * (1 + modifier))
        if use_cache and cache_limit:
        ...
        *,
        ...
        if modifier > best_modifier_value or (
            ...
            best_modifier = CombinationResult(
                ...
                substance_cost=earliest_cost,
                modifier=modifier,
                substances=[ingredients[index][0] for index in earliest_recipe],
                effects=list(state),
            )
        if profit > best_profit_value or (
            ...
            best_profit = CombinationResult(
                sell_price=sell_price,
                substance_cost=cheapest_cost,
                modifier=modifier,
                substances=[ingredients[index][0] for index in cheapest_recipe],
                effects=list(state),
            )
